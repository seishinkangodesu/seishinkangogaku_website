...
- **資料の保管先**に大学アカウント（@hyo-med.ac.jp）は**使えない**。
  学外共有が管理者により禁止されており、サイト訪問者が資料を閲覧できないため。
  保管先は領域名義のGoogleアカウント（`seishinkangodesu@gmail.com`）。
- **投入する側**が大学アカウントを使えるかは**未検証**。外部フォームへのアップロードも
  同じポリシーで止められる可能性がある。ブロックされる場合は個人のGmailを使う。
